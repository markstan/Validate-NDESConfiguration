...
function Install-RSATAD {
    ...
    Write-Output "RSAT-AD-PowerShell is not installed. This Windows Feature is required to continue. This is a requirement for AD tests. Install now? [y/n]"
    New-LogEntry  "Prompt: RSAT-AD-PowerShell is not installed. This Windows Feature is required to continue. This is a requirement for AD tests. Install now?" 2
...
function Get-TCAInfo {
    ...
    try {
        ...
        if ($output) {
            Write-Output $output
	    New-LogEntry "TCAInfo" -Severity 2
        } else {
            Write-Output "cannot fetch the published template details."
	    New-LogEntry "cannot fetch the published template details." -Severity 2 
        }
    } catch {
        Write-Error "An error occurred: $_" -Severity 2
    }
}
...
foreach ($service in $services) {
	...
	New-LogEntry "$service is $($serviceStatus.Status)" -Severity 3
    } else {
        Write-output "$service not found"
	New-LogEntry "$service not found" -Severity 1
    }
}
}

function Get-PFXConnectorAccount {
#Checking the status of PFX Certificate connector running status
Write-StatusMessage "Checking the `"Log on As`" for PFX Certificate Connector for Intune"  
...
if ($service) {
    ...
    if ($serviceProcess.StartName -eq "LocalSystem") {
	    ...
	    New-LogEntry "$($service.Name) is running as Local System" -Severity 3
    } else {
        Write-Output "$($service.Name) is running as $($serviceProcess.StartName)"  
	    New-LogEntry "$($service.Name) is running as $($serviceProcess.StartName)" -Severity 3
    }
} else {
...
#Connectivity the connectivity to autoupdate.msappproxy.net
...
try {
    ...
    if ($connectionTest) {
        ...
        New-LogEntry "Connection to $uniqueURL on port $port is successful."  -Severity 2
    } else {
        Write-output "Connection to $uniqueURL on port $port failed." 
        New-LogEntry "Connection to $uniqueURL on port $port failed." -Severity 2
    }
}
catch {
    Write-output "Error connecting to $uniqueURL. Please test that the service account has internet access."
    New-LogEntry "Error connecting to $uniqueURL. Please test that the service account has internet access." -Severity 2
}
}

function Check-ConnectorCertificate {
#Checking the validity of the Microsoft Intune ImportPFX Connector Certificate"
Write-StatusMessage "Checking Microsoft Intune ImportPFX Connector Certificate"
...
if ($matchingCerts.Count -gt 0) {
    if ($matchingCerts.Count -gt 1) {
        ...
        New-LogEntry "$($matchingCerts.Count) certificates issued by '$issuer' found." -Severity 2
    }
    ...
    foreach ($cert in $matchingCerts) {
        ...
        if ($validToDate -gt (Get-Date)) {
            ...
            New-LogEntry "Certificate is valid from: $($validFromDate) until: $($validToDate)" -Severity 2
        } else {
            Write-output "Certificate is expired! Expiration date: $($validToDate)"
            New-LogEntry "Certificate is expired! Expiration date: $($validToDate)" -Severity 2
        }
    }
} else {
    Write-Output "No certificate issued by '$issuer' found."
    New-LogEntry "No certificate issued by '$issuer' found." -Severity 2
}
}
...
Get-TCAInfo
Get-Services
Get-PFXConnectorAccount
Test-Connectivity
Check-ConnectorCertificate
...
Write-StatusMessage "Checking NDES Install Paramaters..."  
...
if ($WriteLogOutputPath) {
        ...
        if ($PSCmdlet.ParameterSetName -eq "Unattended"  ){
            ...
            Copy-Item $LogFilePath $copyPath
            }
